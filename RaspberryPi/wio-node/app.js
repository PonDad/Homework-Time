...
function timer_stop(){
  ...
  exec(`curl -k -X POST  ${adress}/display_point/1${access_token} |curl -k -X POST  ${adress}/display_digits/0/`+ countTime +`${access_token} | curl -X POST -H "Content-Type: application/json" -d '{"value1":"${stopMoment}","value2":"${startMomentTime}","value3":"${stopMomentTime}"}' https://maker.ifttt.com/trigger/yolanda_kumon_true/with/key/xxxxxxxxxxxxxxxxxxxxx`, (error, stdout, stderr) => {
    if (error) {
      console.error(`exec error: ${error}`);
      return;
    }
